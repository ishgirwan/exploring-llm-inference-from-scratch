...
A closing section steps back from the matmul to the broader question of *which
language* to write kernels in — the 2026 spectrum from Triton to CuTe DSL to raw
CUDA, and what forced it to grow so crowded.
...
| 3 | [Choosing a kernel language](03_kernel_languages_landscape.md) | The kernel-authoring language landscape: the single productivity-vs-control ladder (`torch.compile` → Triton → CuTe DSL/TileLang/ThunderKittens/Gluon/TLX → CUTLASS/CUDA), the "three knobs" that decode any new DSL, the 2021→2026 timeline of how Hopper/Blackwell (WGMMA, TMA, warp specialization, FP8) forced the crowded middle, and what to actually use — Triton first, CuTe DSL for depth (where FlashAttention-4 lives) |
